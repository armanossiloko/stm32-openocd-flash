...
"${OPENOCD_CMD[@]}" -f interface/stlink.cfg -f target/stm32f1x.cfg -c "program $ELF_PATH verify reset exit"
status=$?

if [[ $status -ne 0 ]]; then
  echo
  echo "Retry with NRST held during connect..."
  "${OPENOCD_CMD[@]}" -f interface/stlink.cfg -c "reset_config srst_only srst_nogate connect_assert_srst" -f target/stm32f1x.cfg -c "program $ELF_PATH verify reset exit"
  status=$?
fi
